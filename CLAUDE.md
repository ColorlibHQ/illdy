...
- **The block widget editor is supported, and its previews need help.** `illdy_is_legacy_widget_preview()` detects `REST_REQUEST && IFRAME_REQUEST`, which is what `WP_REST_Widget_Types_Controller::render_legacy_widget_preview_iframe()` sets while building a full front-end document for each Legacy Widget block. Two things hang off it, both in `functions.php`:
  1. `illdy_needs_front_page_assets()` returns true there, so Owl Carousel, countTo, jQuery Visible and jQuery UI load. Without them the counter and skill widgets render empty elements that JS is supposed to fill.
  2. `illdy_legacy_widget_preview_styles()` adds inline CSS: colour fixes (the counter and testimonial widgets are white text supplied by the `#counter` / `#testimonials` section wrappers, absent around a single widget, so they were white on white), `:empty` fallbacks for the counter number and skill bar, and a `:before` label per widget carrying its registered name — a column of previews otherwise gives no clue which widget is which.
  3. `illdy_trim_legacy_widget_preview_assets()` dequeues the preloader, lightbox, sticky header, parallax and `scripts.min.js`. One iframe is rendered **per widget**, so a populated front page means 20+ full front-end documents on one admin screen. Nothing declares these as dependencies, so removing them can't break the render — but don't strip `illdy-plugins` or its deps, which are what draw the counter and skill bar.

  The widget **forms** need nothing special: the Companion hooks `admin_footer-widgets.php` (which the block screen also fires) and initialises on the `widget-added` / `widget-updated` jQuery events, which the Legacy Widget block triggers after injecting form HTML. Verified: icon picker, colour picker and TinyMCE all initialise in the block editor.
- **jQuery Migrate is kept by default, on purpose.** Neither the theme nor the Companion calls an API it patches — every `.bind()` in the theme's Customizer JS is `wp.customize`'s own method, not jQuery's deprecated one. It still loads, because a theme can't know what a site's plugins call and a missing Migrate turns a silent deprecation into a fatal. `illdy_needs_jquery_migrate()` drops it only inside widget previews (which run just our code, 20+ times per screen); site owners opt out everywhere with `add_filter( 'illdy_load_jquery_migrate', '__return_false' )`. Note `wp-admin` is deliberately exempt. When diagnosing, `JQMIGRATE: Migrate is installed` alone means nothing is using it — only `JQMIGRATE: <warning>` lines matter.
