...
class Epsilon_Welcome_Screen {
	...
	private function get_plugin_information( $slug = '' ) {
		...
		// Check if info is a WP_Error object
	...
	private function call_plugin_api( $slug = '' ) {
		...
		if ( false === $call_api ) {
			...
			if (!is_wp_error($call_api)) {
				// Make sure author information is preserved as HTML
				if (isset($call_api->author) && strpos($call_api->author, '<a href') !== false) {
					$call_api->author = wp_kses_post($call_api->author);
				}
	...
	private function check_for_icon( $object ) {
		// Check if $object is null or not an array
		...
		if ( ! empty( $object['svg'] ) ) {
			...
			$plugin_icon_url = isset($object['default']) ? $object['default'] : '';
		}
